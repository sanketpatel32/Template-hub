# NestJS Template Details

Starter onboarding lives in [`README.md`](./README.md).  
Agent workflow and guardrails live in [`AGENTS.md`](./AGENTS.md).

## Architecture Overview
This project is a production-oriented NestJS + TypeScript + Bun backend template.

### Core Structure
- `src/app.ts`: Nest app composition and middleware order.
- `src/app.module.ts`: Root Nest module containing route controllers.
...
- `src/middleware/`: Request ID/context, rate-limiting, and error middleware/filter.
- `src/observability/`: Structured logging, HTTP logs, tracing scaffold, metrics.
- `src/routes/`: Controller modules (`health`, `ready`, `metrics`, `ping`).
