...
def install_signal_handlers(loop: asyncio.AbstractEventLoop, lifecycle: ServerLifecycle) -> None:
    for sig in (signal.SIGINT, signal.SIGTERM):
        loop.add_signal_handler(
            sig,
            lambda reason=sig.name: loop.create_task(lifecycle.shutdown(reason, 0)),  # type: ignore[misc]
        )
