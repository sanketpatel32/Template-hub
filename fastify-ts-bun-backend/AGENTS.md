...
## Project Purpose
This repository is a production-focused Fastify + TypeScript + Bun backend template.  
...
## Architecture Map
- `src/app.ts`: Fastify app composition and hook/plugin order.
...
- HTTP logs are emitted through Fastify hooks and `pino`.
...
## Testing Policy
- Route behavior: integration tests with Fastify `inject`.
