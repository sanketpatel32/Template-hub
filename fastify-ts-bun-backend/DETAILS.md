# Fastify Template Details

Starter onboarding lives in [`README.md`](./README.md).  
Agent workflow and guardrails live in [`AGENTS.md`](./AGENTS.md).

## Architecture Overview
This project is a production-oriented Fastify + TypeScript + Bun backend template.

### Core Structure
- `src/app.ts`: Fastify app composition and plugin/hook order.
...
- `src/types/`: Shared API/context/Fastify typing.
...
## Logging and Tracing
- Structured app/request logs use `pino` and Fastify request logging hooks.
...
| `TRUST_PROXY` | boolean | `true` | Fastify trust proxy setting |
